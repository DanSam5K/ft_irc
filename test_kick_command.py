...
def client1():
    """First client: channel operator who will kick user2"""
    ...
    # Kick user2 from the channel
    sock.send(b"KICK #test user2 :You are kicked!\r\n")
    time.sleep(0.1)
    
    # Wait for kick to be processed
    time.sleep(2)
...
def client2():
    """Second client: will be kicked from channel"""
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.connect(('127.0.0.1', 6667))
...
def main():
    print("Starting KICK command test...")
    ...
    print("KICK command test completed!")
