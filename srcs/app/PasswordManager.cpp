...
PasswordManager::PasswordManager( const std::string &connectionPassword ) {
	...
	{
		log_event::info( "PasswordManager: No password provided, using default password" );
...
std::string PasswordManager::computeSHA256( const std::string &password )
{
	...
	log_event::info( "PasswordManager: Created hash", ss.str() );
	return ( ss.str() );
}
...
void PasswordManager::verifyPassword( const std::string &hash, const std::string &plainText )
{
	...
	{
		log_event::info( "PasswordManager: Incorrect password..." );
		throw InvalidPasswordException();
	}
	log_event::info( "PasswordManager: Passwords match!" );
}
